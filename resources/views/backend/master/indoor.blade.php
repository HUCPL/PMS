  ...
  @csrf
    <div class="container-xxl flex-grow-1 container-p-y">
      <div class="row">
         ...
         <div class="col-md-6">
            <div class="card mb-4">
              <h5 class="card-header">Indoor Name</h5>
         ...
         <div class="col-md-6">
            <div class="card mb-4" style="overflow:scroll; -">
              <h5 class="card-header">Indoor Lists</h5>
              <div class="card-body demo-vertical-spacing demo-only-element">
                @if ($indoorList->isNotEmpty())
                <table class="table">
                    <thead>
                      <tr>
                        <th scope="col">SrNo.</th>
                        <th scope="col" style="white-space: nowrap">indoor Name</th>
                    ...
                    <tbody>
                      ...
                      @php
                       $i = 1;
                      @endphp
                      @foreach ($indoorList as $features)
                        <tr>
                          <th scope="row" style="font-size:1rem">{{ $i++ }}</th>
                          <td style="font-size:1rem">{{ $features->indoor_name }}</td>
                          ...
                          <td style="white-space: nowrap;"><a class="btn btn-danger" style="color:white;" href="{{ route('deletePropertyIndoor',['id'=>$features->indoor_id]) }}"><i class="fa-solid fa-trash"></i></a>&nbsp;<a class="btn btn-primary indoorUpdate" style="color:white;" data-bs-toggle="modal" data-bs-target="#modalCenter" data-name="{{ $features->indoor_name}}" data-category = "{{ $features->master_category}}" data-id="{{ $features->indoor_id  }}" ><i class="fa-solid fa-pen-to-square"></i></a></td>
                        </tr>
                      @endforeach
                ...
                @else
                <div class="alert alert-danger text-center">Sorry indoor List Not Exist</div>
                @endif
              </div>
              {{ $indoorList->links() }}
            </div>
...
<div class="col-lg-4 col-md-6">
  <div class="mt-3">
    ...
    <div class="modal fade" id="modalCenter" tabindex="-1" aria-hidden="true">
      <form action="{{ route('updatePropertyIndoor') }}" method="post" enctype="multipart/form-data">
        @csrf
